...
class TestCookieLogParser:
    def test_init_invalid_date(self, get_file_path):
        ...
        with pytest.raises(IllegalArgumentException):
            CookieLogParser(date=date, path=path)
    ...
    def test_parse_log_and_get_most_active(self, get_file_path):
        path = get_file_path("cookie_log.csv")
        date = "2018-12-09"
        parser = CookieLogParser(date=date, path=path)
        cookies = parser.parse_log_and_get_most_active_cookie()
    ...
    def test_parse_logs_and_get_most_active(self, get_source_dir):
        path = str(get_source_dir)
        date = "2018-12-06"
        parser = CookieLogParser(date=date, path=path)
        cookies = parser.parse_log_and_get_most_active_cookie()
        assert cookies == ["SAZuXPGUrfbcn5UA"]
